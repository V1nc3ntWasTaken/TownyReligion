...
public class Settings {
	...
	public static boolean loadSettingsAndLang() {
		TownyReligion townyReligion = TownyReligion.getTownyReligion();
		try {
			Settings.loadConfig(townyReligion.getDataFolder().getPath() + File.separator + "config.yml", townyReligion.getVersion());
		} catch (IOException e) {
            e.printStackTrace();
            TownyReligion.severe("Config.yml failed to load!");
            return false;
        }

		try {
			Translation.loadLanguage(townyReligion.getDataFolder().getPath() + File.separator, "english.yml");
		} catch (IOException e) {
	        e.printStackTrace();
	        TownyReligion.severe("Language file failed to load!");
	        return false;
	    }
	...
	public static void loadConfig(String filepath, String version) throws IOException {
		if (FileMgmt.checkOrCreateFile(filepath)) {
			...
			if (!config.load())
				TownyReligion.severe("Failed to load Config!");
	...
	private static void sendError(String msg) {
		...
		TownyReligion.severe("Error could not read " + msg);
	}
