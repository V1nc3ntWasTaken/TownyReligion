...
public final class Translation {
	...
	public static void loadLanguage(String filepath, String defaultRes) throws IOException {
		...
		try {
			...
			TownyReligion.info("Lang: Custom language file detected, not updating.");
			TownyReligion.info("Lang: " + res + " v" + Translation.of("version") + " loaded.");
			return;
		} catch (InvalidConfigurationException e) {
			TownyReligion.severe("Invalid Configuration in language file detected.");
		}
		...
		if (!langVersion.equalsIgnoreCase(resVersion)) {
			...
			TownyReligion.info("Lang: Language file replaced with updated version.");
	...
	public static String of(String key) {
		...
		if (data == null) {
			TownyReligion.severe("Error could not read " + key.toLowerCase() + " from " + Settings.getString(ConfigNodes.LANGUAGE));
			return "";
		}
